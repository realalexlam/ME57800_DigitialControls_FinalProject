...
class RpmOnlyStraighteningController(Node):
    def __init__(self) -> None:
        super().__init__("rpm_only_straightening_controller")
        ...
        self.get_logger().info(f"RPM-only controller started on port {self.port}")
...
def main(args=None) -> None:
    ...
    node = RpmOnlyStraighteningController()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info("Shutting down RPM-only controller")
    finally:
        node.destroy_node()
        rclpy.shutdown()
